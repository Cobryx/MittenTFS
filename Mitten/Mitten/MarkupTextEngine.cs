...
namespace Mitten {
  internal struct FormatInstruction {
    ...
    public FormatInstruction(SpriteFont font, Color color, float scaling=1) {
      Font = font;
      Color = color;
      Scaling = scaling;
    }
  ...
  internal struct CompiledTextElement : ICompiledElement {
    ...
    public CompiledTextElement(string text, Vector2 position, FormatInstruction formatInstruction)
      ...
      size = formatInstruction.Font.MeasureString(text) * formatInstruction.Scaling;
    }
    ...
    public void Draw( Vector2 offset) {
      ...
      Globals.spriteBatch.DrawString(formatInstruction.Font, text, offset + Position, formatInstruction.Color, 0, origin, Vector2.One*formatInstruction.Scaling, SpriteEffects.None, 0);
    }
  ...
  internal struct CompiledImageElement : ICompiledElement {
    ...
    public void Draw(Vector2 offset) {
      var origin = new Vector2(0, size.Y / 2f);
      Globals.spriteBatch.Draw(image, offset + Position, null, color, 0, origin, scale, SpriteEffects.None, 0);
    }
  ...
  internal struct CompiledVideoElement : ICompiledElement {
    ...
    public void Draw(Vector2 offset) {
      var origin = new Vector2(0, size.Y / 2f);
      Globals.spriteBatch.Draw(videoPlayer.GetTexture(), offset + Position, null, color, 0, origin, scale, SpriteEffects.None, 0);
    }
  ...
  public class MarkupTextEngine {
    ...
    private readonly Func<string, Video> VideoResolver;
          ...
          case XmlNodeType.Element:
            switch (reader.Name) {
              case "text": {
                ...
                s = reader.GetAttribute("align");
                if (!string.IsNullOrEmpty(s))
                {
                    
                }
                float scaling=1;
                s = reader.GetAttribute("dim");
                if (!string.IsNullOrEmpty(s))
                {
                    scaling = float.Parse(s);
                    //font.LineSpacing = 1000;// pixels;
                ...
                formatingStack.Push(new FormatInstruction(font, color, scaling));
              }
                break;
              case "if": {
