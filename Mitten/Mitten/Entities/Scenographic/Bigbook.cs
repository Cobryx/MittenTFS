...
namespace Mitten
{
    public class Bigbook:InteractiveScenography
    {
        Random r;
        public Bigbook(Vector2 position, float rotation, int type, int subtype, ref SpriteSheet[] sheet, ref Dungeon dungeon)
            : base(position, rotation, type, subtype, ref sheet, ref dungeon)
        {
            ...
            sheetIndex = (int)sheetIndexes.bigbook;
            /*switch (r.Next(1, 9))
          {
