...
namespace Mitten
{
    public class Throne:InteractiveScenography
    {
        Random r;
        public Throne(Vector2 position, float rotation, int type, int subtype, ref SpriteSheet[] sheet, ref Dungeon dungeon)
            : base(position, rotation, type, subtype, ref sheet, ref dungeon)
        {
            ...
            if (subtype != 0)
            {
                switch (subtype)
                {
                    case 1: sheetIndex = (int)sheetIndexes.throne1; break;
                    case 2: sheetIndex = (int)sheetIndexes.throne2; break;
                }
            }
            else
                switch (r.Next(1, 3))
