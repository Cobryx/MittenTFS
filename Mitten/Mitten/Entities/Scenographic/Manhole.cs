...
namespace Mitten
{
    public class Manhole : InteractiveScenography
    {
        Random r;
        public Manhole(Vector2 position, float rotation, int type, int subtype, ref SpriteSheet[] sheet, ref Dungeon dungeon)
            : base(position, rotation, type, subtype, ref sheet, ref dungeon)
        {
            ...
            if (subtype != 0)
            {
                switch (subtype)
                {
                    case 1: sheetIndex = (int)sheetIndexes.manhole1; break;
                    case 2: sheetIndex = (int)sheetIndexes.manhole2; break;
                    case 3: sheetIndex = (int)sheetIndexes.manhole3; break;
                }
            }
            else
                switch (r.Next(1, 4))
