...
namespace Mitten
{
    public class SleepBag : InteractiveScenography
    {
        Random r;
        public SleepBag(Vector2 position, float rotation, int type, int subtype, ref SpriteSheet[] sheet, ref Dungeon dungeon)
            : base(position, rotation, type, subtype, ref sheet, ref dungeon)
        {
            ...
            if (subtype != 0)
            {
                switch (subtype)
                {
                    case 1: sheetIndex = (int)sheetIndexes.sleepbag1; break;
                    case 2: sheetIndex = (int)sheetIndexes.sleepbag2; break;
                }
            }
            else
                switch (r.Next(1, 3))
