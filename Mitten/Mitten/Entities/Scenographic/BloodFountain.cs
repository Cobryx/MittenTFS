...
namespace Mitten
{
    public class BloodFountain : InteractiveScenography,IShadow
    {
        Random r;
        public BloodFountain(Vector2 position, float rotation, int type, int subtype, ref SpriteSheet[] sheet, ref Dungeon dungeon)
            : base(position, rotation, type, subtype, ref sheet, ref dungeon)
        {
            ...
            sheetIndex = (int)sheetIndexes.bloodfountain;
            /*switch (r.Next(1, 9))
            {
            ...
            shadow = Krypton.ShadowHull.CreateRectangle(new Vector2(boundingBox.HalfWidths.X*2f, boundingBox.HalfWidths.Y*2f));//.CreateRectangle(new Vector2(1, 28)); //definizione dell ombra, da rivedere con un poligono più dettagliato
            shadow.Axis = axis;
