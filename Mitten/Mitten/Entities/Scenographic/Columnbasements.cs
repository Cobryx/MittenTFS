...
namespace Mitten
{
    public class Columnbasements:InteractiveScenography,IShadow
    {
        Random r;
        public Columnbasements(Vector2 position, float rotation, int type, int subtype, ref SpriteSheet[] sheet, ref Dungeon dungeon)
            : base(position, rotation, type, subtype,ref sheet,ref dungeon)
        {
            ...
            if (subtype != 0)
            {
                switch (subtype)
                {
                    case 1: sheetIndex = (int)sheetIndexes.columnbasements1; break;
                    case 2: sheetIndex = (int)sheetIndexes.columnbasements2; break;
                    case 3: sheetIndex = (int)sheetIndexes.columnbasements3; break;
                }
            }
            else
                switch (r.Next(1, 4))
