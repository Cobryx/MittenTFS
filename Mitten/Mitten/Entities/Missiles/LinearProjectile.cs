...
namespace Mitten
{
    public class LinearProjectile : Throwable,IAttacker
    {
        ItemInfo item;

        public LinearProjectile(Vector2 position, float radius, Vector2 direction, float speed, float depth, float health, float rotation, int factionId, int type, int subtype,
            ...
            : base(position, radius, direction, speed, depth, health, rotation, factionId, type, subtype, ref sheet, ref dungeon)
        {
            ...
            status.SetOn((int)th_states.launched, (int)th_states.launched, true, true);
            item = originalItem;
