...
namespace Krypton.Lights
{
    public class Light2D : ILight2D
    {
        ...
        public Vector2 Position 
        {
            ...
            set { this.mPosition = new Vector2(value.X, -value.Y); }
        }

        /// <summary>
        /// Posizione dell'entità emettitrice di luce
        /// </summary>
        ...
        public void Draw(KryptonRenderHelper helper, List<ShadowHull> hulls)
        {
            ...
            foreach (ShadowHull hull in hulls)
            {
                ...
                helper.Effect.Parameters["LightAxis"].SetValue(new Vector2(axis.Bottom, axis.Top));
                ...
                //ottimizzabile
                if (hull.Visible && Light2D.IsInRange(hull.Position - this.Position, hull.MaxRadius * Math.Max(hull.Scale.X, hull.Scale.Y) + this.Range)) //debug spegnimento torce da rivedere
                {
                //    if (this.Axis.Bottom > hull.Axis.Bottom) //debug: da testare e rivedere
                    {
                        helper.BufferAddShadowHull(hull);
                        helper.Effect.Parameters["ShadowPosition"].SetValue(hull.Position);
                        helper.Effect.Parameters["ShadowAxis"].SetValue(new Vector2(hull.Axis.Bottom, hull.Axis.Top));
        ...
        /// <returns></returns>
        public static bool IsInRange(Vector2 offset, float dist)
        {
